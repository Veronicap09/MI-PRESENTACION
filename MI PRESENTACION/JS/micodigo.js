...
function impares (){
    const impar=[];
    let n= prompt("CUANTOS NUMEROS IMPARES DESEA GENERAR");
    for(let i=0;i<n;i++){
        impar[i]=(2*i)+1;
        console.log(impar[i]);
    }
...
function multiplosdecinco(){
    const mult = [];
    let n = prompt("¿CUANTOS NUMEROS MULTIPLOS DE 5 DESEA GENERAR?");
    for (let i = 1; i<=n; i++){
        mult[i]=(i*5);
        console.log(mult[i]);
    }
...
function multiplosdetres(){
    const multiplos = [];
    let n = prompt("CUANTOS MULTIPLOS DE 3 DESEA GENERAR");
    for (let j = 1; j<=n; j++){
        multiplos[j]=j*3;
